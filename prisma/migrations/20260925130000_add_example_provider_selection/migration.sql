-- Lets the owner pick which ExampleProvider (src/lib/example-provider.ts)
-- writes a generated model answer -- "gemini" (the existing native
-- integration, unchanged) or "openrouter" (the same unified gateway adapter
-- already available for essay correction) -- independently of the model
-- string, from the admin panel. A null value falls back to "gemini", so an
-- existing deployment with no row here (or an existing row with this column
-- null) behaves exactly as before.
--
-- Deliberately a plain nullable TEXT column with no CHECK constraint here,
-- matching correctionProvider (see 20260925120000_add_correction_provider_selection)
-- and this table's existing correctionModel/exampleModel columns: the
-- app-level EXAMPLE_PROVIDER_IDS union (validated at the admin API route) is
-- the sole source of truth, so a future provider adapter never needs a
-- migration to this table.
ALTER TABLE "AppConfig" ADD COLUMN "exampleProvider" TEXT;

-- Widens the AdminEvent closed vocabulary so an EXAMPLE_PROVIDER_FAILED row
-- may now also record "openrouter", mirroring the ExampleProvider ids above
-- (CORRECTION_PROVIDER_FAILED already allows it, see
