-- Adds the closed-vocabulary event type/provider for the self-hosted
-- LanguageTool grammar checker, mirroring CORRECTION_PROVIDER_FAILED and
-- TRANSLATION_PROVIDER_FAILED. Purely additive: every existing row still
-- satisfies every constraint below unchanged, since only new allowed values
-- are introduced, none removed or narrowed.
...
ALTER TABLE "AdminEvent"
    ...
    ADD CONSTRAINT "AdminEvent_provider_check" CHECK (
        "provider" IS NULL OR "provider" IN ('gemini', 'deepl', 'unofficial', 'deepl_or_unofficial', 'languagetool')
    );
