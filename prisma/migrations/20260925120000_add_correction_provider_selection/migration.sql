-- Lets the owner pick which CorrectionProvider (src/lib/correction-provider.ts)
-- grades an essay -- "gemini" (the existing native integration, unchanged)
-- or "openrouter" (a new unified gateway adapter covering other models) --
-- independently of the model string, from the admin panel. A null value
-- falls back to "gemini", so an existing deployment with no row here (or an
-- existing row with this column null) behaves exactly as before.
--
-- Deliberately a plain nullable TEXT column with no CHECK constraint here,
-- matching this table's existing correctionModel/exampleModel columns: the
-- app-level CORRECTION_PROVIDER_IDS union (validated at the admin API route)
-- is the sole source of truth, so a future provider adapter never needs a
-- migration to this table.
ALTER TABLE "AppConfig" ADD COLUMN "correctionProvider" TEXT;

-- Widens the AdminEvent closed vocabulary so a CORRECTION_PROVIDER_FAILED
-- row may now also record "openrouter", mirroring the CorrectionProvider ids
-- above. EXAMPLE_PROVIDER_FAILED is untouched -- example generation stays
-- Gemini-only for now (see the CorrectionProvider rollout notes).
...
ALTER TABLE "AdminEvent"
    ADD CONSTRAINT "AdminEvent_provider_check" CHECK (
        "provider" IS NULL OR "provider" IN (
            'gemini', 'openrouter', 'deepl', 'unofficial', 'deepl_or_unofficial', 'languagetool', 'hunspell'
        )
    );
