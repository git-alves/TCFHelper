  ...
  return (
    <main className="bg-[#080808] text-[#f5f5f5]">
      <section className="mx-auto flex min-h-[calc(100svh-65px)] max-w-3xl flex-col items-center justify-center px-6 py-20 text-center sm:px-10">
        ...
        <Link href={destination} className="mt-8 rounded-full bg-[#f5f5f5] px-7 py-3 text-base font-medium text-[#111] transition-transform transition-colors hover:scale-[1.02] hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white">
          {copy.primaryAction}
        </Link>
      </section>
      ...
      <section className="border-y border-white/[.12] bg-white/[.035] px-6 py-20 sm:px-10" aria-labelledby="proof-heading">
        <div className="mx-auto max-w-5xl">
          <p className="text-sm font-medium text-violet-300">{copy.proofEyebrow}</p>
          <h2 id="proof-heading" className="mt-3 max-w-xl text-3xl font-semibold tracking-tight sm:text-4xl">{copy.proofTitle}</h2>
          <div className="mt-10 grid gap-4 md:grid-cols-2">
            <article className="rounded-2xl border border-white/[.12] bg-black/20 p-6"><p className="text-sm font-medium text-zinc-400">{copy.before}</p><p className="mt-5 text-lg leading-8 text-zinc-300">{copy.beforeText}</p></article>
            <article className="rounded-2xl border border-violet-300/35 bg-violet-400/[.08] p-6"><p className="text-sm font-medium text-violet-200">{copy.after}</p><p className="mt-5 text-lg leading-8 text-white">{copy.afterText}</p></article>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-6 py-20 sm:px-10" aria-labelledby="problem-heading">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {copy.skills.map((skill) => <article key={skill.title} className="rounded-2xl border border-white/[.12] p-5"><h3 className="font-semibold">{skill.title}</h3><p className="mt-2 text-sm leading-6 text-zinc-400">{skill.description}</p></article>)}
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-5xl gap-10 px-6 py-20 sm:px-10 md:grid-cols-2" aria-labelledby="method-heading">
        <div><p className="text-sm font-medium text-violet-300">{copy.methodEyebrow}</p><h2 id="method-heading" className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">{copy.methodTitle}</h2><p className="mt-5 leading-7 text-zinc-400">{copy.methodDescription}</p></div>
        <ul className="space-y-3 self-center">{copy.methodPoints.map((point) => <li key={point} className="rounded-xl border border-white/[.12] px-5 py-4 text-zinc-200">{point}</li>)}</ul>
      </section>
      ...
      <section className="mx-auto max-w-3xl px-6 py-20 sm:px-10" aria-labelledby="faq-heading">
        <p className="text-sm font-medium text-violet-300">{copy.faqEyebrow}</p><h2 id="faq-heading" className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">{copy.faqTitle}</h2>
        <div className="mt-10 divide-y divide-white/[.12]">{copy.faqs.map((faq) => <details key={faq.question} className="group py-5"><summary className="cursor-pointer list-none font-medium text-white marker:hidden">{faq.question}<span className="float-right text-violet-200 transition-transform group-open:rotate-45" aria-hidden="true">+</span></summary><p className="mt-3 max-w-2xl leading-7 text-zinc-400">{faq.answer}</p></details>)}</div>
      </section>
