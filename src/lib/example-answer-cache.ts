 ...
 *
 * `providerFingerprint` is the same idea for AppConfig.exampleProvider/
 * exampleModel (see app-config.ts): without it, switching the admin's
 * selected ExampleProvider (or its model override) would still return an
 * already-cached answer generated by the previously selected one, since
...
export function hashExampleTopic(
  ...
  return createHash("sha256")
    .update(
      `${MODEL_ANSWER_PROMPT_VERSION}\n${taskType}\n${topicPrompt}\n${promptOverridesFingerprint}\n${providerFingerprint}`,
      "utf8",
    )
    .digest("hex");
}
