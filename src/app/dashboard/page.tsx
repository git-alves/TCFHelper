...
import { PracticeProgressCard } from "@/components/practice-progress-card";
...
import { getPracticeProgressSummary } from "@/lib/practice-progress";
...
export default async function DashboardPage() {
  ...
  const [points, recentCorrections, practiceProgress, locale] = await Promise.all([
    getEssayProgressPoints(user.id),
    getRecentCorrections(user.id),
    getPracticeProgressSummary(user.id),
    getRequestLocale(),
  ]);
  const copy = getAppCopy(locale);
  const hasGettingStarted =
    points.length === 0 && recentCorrections.length === 0 && practiceProgress.completedExercises === 0;
