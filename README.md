...
The Prisma Client is generated into the default `node_modules/@prisma/client`
location (regenerated by `postinstall` and `npm run db:generate`), not a
custom path — see the comment on the `generator client` block in
`prisma/schema.prisma` for why that matters on Vercel.
