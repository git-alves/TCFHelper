...
- Completion presents two distinct repeat actions: **Replay with new variants** keeps the fixed six-stage delivery order and selects a different approved variant within a stage when available; **Start fresh** creates a new curated session without promising different variants. Switching task, level, or topic clears the local unfinished snapshot. A page refresh offers Resume/Discard rather than implying a server save.
