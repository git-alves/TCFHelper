...
| `ESSAY_SERVICE` | `ERROR` | `GRAMMAR_CHECK_PROVIDER_FAILED` | Local user ID, the `languagetool` provider, and a fixed failure class/status (`not_configured`, `transport_error`, `upstream_http_error`, or `provider_unavailable`). Recorded by `/api/language-check` (see [French grammar and spelling check](french-grammar-check.md)); the checked draft text never enters the event, and a client-cancelled (aborted) request never does either. |
