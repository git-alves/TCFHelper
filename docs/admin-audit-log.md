...
| `ESSAY_SERVICE` | `ERROR` | `SPELL_CHECK_FAILED` | Local user ID, the bundled `hunspell` provider, and the fixed `provider_unavailable` reason/status. Recorded by `/api/spell-check` (see [French spelling check](french-spell-check.md)); the checked draft text and exception text never enter the event. |
