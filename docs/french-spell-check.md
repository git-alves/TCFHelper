...
The 20,000-character request limit matches the editor. The endpoint requires
an activated account, has an in-memory per-user rate limit, validates a strict
payload, and marks responses `private, no-store`.
